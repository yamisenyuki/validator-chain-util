...
describe("validatorWrapper Sync",()=>{
  ...
  test("string",()=>{
    ...
    expect(valid("testStr").notEmpty().contains("test").run(obj)).toEqual({pass:true,msg:"",data:[]})
    expect(valid("testStr").notEmpty().isArray({min:4}).run(obj)).toEqual({pass:false,msg:"_isArray",data:[
  ...
  test("array",()=>{
    ...
    expect(valid("testObjArray").isArray({min:2,max:2}).bail().run(obj)).toEqual({pass:true,msg:"",data:[]})
  })
  ...
  test("all function",()=>{
    expect(valid().allowNull().run(null)).toEqual({pass:true,msg:"",data:[]})
    expect(valid().equals("test").run("test")).toEqual({pass:true,msg:"",data:[]})
    expect(valid().isAlpha().run("test")).toEqual({pass:true,msg:"",data:[]})
    expect(valid().isAlphanumeric().run("test")).toEqual({pass:true,msg:"",data:[]})
    expect(valid().isAscii().run("test")).toEqual({pass:true,msg:"",data:[]})
    expect(valid().isBase64().run("test")).toEqual({pass:true,msg:"",data:[]})
    expect(valid().isBefore("2019-01-01").run("2018-01-01")).toEqual({pass:true,msg:"",data:[]})
    expect(valid().isBefore("2019-01-01").run("2019-01-01")).toEqual({pass:false,msg:"_isBefore",data:[
      {fun:"_isBefore",msg:"_isBefore",pass:false}
    ]})

    expect(valid().isBoolean().run(true)).toEqual({pass:true,msg:"",data:[]})
    expect(valid().isBoolean().run("test")).toEqual({pass:false,msg:"_isBoolean",data:[
      {fun:"_isBoolean",msg:"_isBoolean",pass:false}
    ]})

    expect(valid().isByteLength({min:1,max:4}).run("test")).toEqual({pass:true,msg:"",data:[]})
    expect(valid().isEmail().run("123@gmail.com")).toEqual({pass:true,msg:"",data:[]})
    expect(valid().isNumeric().run("123")).toEqual({pass:true,msg:"",data:[]})
    expect(valid().isIn(["test","test1"]).run("test")).toEqual({pass:true,msg:"",data:[]})
    expect(valid().isBase32().run("test")).toEqual({pass:false,msg:"_isBase32",data:[
      {fun:"_isBase32",msg:"_isBase32",pass:false}
    ]})
    expect(valid().isBIC().run("test")).toEqual({pass:false,msg:"_isBIC",data:[
      {fun:"_isBIC",msg:"_isBIC",pass:false}
    ]})
    expect(valid().isBtcAddress().run("test")).toEqual({pass:false,msg:"_isBtcAddress",data:[
      {fun:"_isBtcAddress",msg:"_isBtcAddress",pass:false}
    ]})
    expect(valid().isCreditCard().run("test")).toEqual({pass:false,msg:"_isCreditCard",data:[
